...
class PCAEngine:
    ...
    def _build_suggestion_prompt(
        ...
        return (
            ...
            f"TASK: Suggest up to {max_suggestions} alternative ingredient substitutions "
            f"that reduce cost per portion while preserving menu intent.\n\n"
            f"Respond with this exact JSON structure and nothing else:\n"
            f'{{"suggestions": [{{"ingredient_to_replace": "string", '
            f'"alternate_item_key": "string or null", '
            f'"alternate_item_description": "string", '
            f'"alternate_vendor": "string", '
            f'"alternate_cost_per_portion": 0.0, '
            f'"alternate_cost_variance": 0.0, '
            f'"estimated_product_cost_pct": 0.0}}]}}'
        )

    def _call_anthropic(self, prompt: str, api_key: str) -> str:
        """
        Call Claude via the Anthropic SDK and return raw response text.
        Uses claude-haiku for fast, cost-efficient suggestions.
        Falls back to empty suggestions if the SDK is unavailable.
        """
        try:
            import anthropic
        except ImportError:
        ...
        response = client.messages.create(
            ...
            max_tokens=2048,
            system=(
                "You are a food cost analyst for a stadium concession operation. "
                "Always respond with valid JSON only — no markdown, no explanation, "
                "no code fences. Return exactly one JSON object matching the schema provided."
            ),
