# auth.py  —  User Authentication + Identity Resolution  v1.2.0
#
...
# v1.2.0: "✏️ Name" button moved from user badge to User Management panel
#
...
_SERVICE_ACCOUNTS = {
    ...
    "appuser",   # ← Streamlit Cloud server process
}
...
def render_user_badge() -> None:
    ...
    v1.2.0: "✏️ Name" button removed — now in Settings→User Management
    """
    u = get_current_user()
...
def render_user_management(db) -> None:
    ...
    st.caption("Manage roles, display names, PINs, and account status.")
    ...
    for u in users:
        ...
        with st.expander(
            ...
            # ── Display Name ──────────────────────────────────────────
            with col1:
                current_display = u.get("display_name") or uname
                new_display = st.text_input(
                    "Display Name",
                    value=current_display,
                    placeholder="e.g. Tre' Church",
                    key=f"display_{uname}",
                )
                if st.button("Update Name", key=f"display_btn_{uname}"):
                    if new_display.strip():
                        db.upsert_user({"username": uname, "display_name": new_display.strip()})
                        # Update session if this is the current user
                        cur_user = get_current_user()
                        if cur_user.get("username") == uname:
                            st.session_state["current_user"]["display_name"] = new_display.strip()
                        st.success(f"Display name → {new_display.strip()}")
                        st.rerun()
