...
# ────────────────────────────────────────────────────────────────────
# Encoding helper (was previously imported from importer — inlined here)
# ────────────────────────────────────────────────────────────────────
...
class GLCodeManager:
    ...
    def __init__(self, database):
        ...
        # User-configurable matching settings
        self.settings = {
    ...
    def find_best_gl_match(self, description: str) -> Optional[Dict]:
        ...
        for gl_code, info in self.gl_mappings.items():
            for example in info["examples"]:
                ...
                score = (fuzzy * 0.4 + token * 0.6) if self.settings["use_weighted_match"] else max(fuzzy, token)
        ...
        return best if (best and best["confidence"] >= 0.4) else None

    # ────────────────────────────────────────────────────────────────
    # Auto-Assignment
    # ────────────────────────────────────────────────────────────────

    def assign_gl_codes_to_items(self, min_confidence: float = None) -> Dict:
        """Auto-assign GL codes using current settings."""
        threshold = min_confidence if min_confidence is not None else self.settings["min_confidence"]
    ...
    def export_gl_mappings(self, filepath: str) -> bool:
        rows = [
            {
                "GL Code":             gl_code,
                "GL Name":             info["name"],
