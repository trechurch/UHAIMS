...
class FormatDetector:
    ...
    def detect(self, content: bytes, filename: str) -> DetectionResult:
        ...
        if ext == 'pdf':
            ...
            if not rows:
                # Image-only PDF — no extractable tables
                return DetectionResult(
                    FMT_E, 85, FORMAT_LABELS[FMT_E],
                    ["Image PDF — no extractable tables found. OCR not yet implemented."],
                )
            # Text-layer PDF — fall through to normal format scoring below
            scores = {
            ...
            notes = ["Text-layer PDF. Scores: " + ", ".join(f"{k}={v}" for k, v in scores.items())]
            if best_score < 40:
                return DetectionResult(FMT_UNKNOWN, best_score, FORMAT_LABELS[FMT_UNKNOWN], notes)
            return DetectionResult(best_fmt, best_score, FORMAT_LABELS[best_fmt], notes)
